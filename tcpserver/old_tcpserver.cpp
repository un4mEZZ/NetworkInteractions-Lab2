...
int main(int argc, char *argv[]) {
    ...
    //ensure the server can reliably bind to the port after recent termination
    int opt = 1;
    ...
    //max value of int backlog defined in socket.h
    ...
    while (!should_stop) {
        int event_count = poll(pfd, client_count + 1, 1000);
        if  (event_count < 0) {
            printf("Poll error: %s\n", strerror(errno));
            break;
        }
        ...
        if (pfd[0].revents & POLLIN) {
            ...
            if (new_client >= 0 && client_count < MAX_CLIENTS) {
                ...
                pfd[client_count + 1].fd = new_client;
                pfd[client_count + 1].events = POLLIN | POLLOUT;
        ...
        for (int i = 0; i < client_count; i++) {
            if (pfd[i + 1].revents & (POLLHUP | POLLERR)) {
                ...
                memmove(&pfd[i + 1], &pfd[i + 2], (client_count - i - 1) * sizeof(struct pollfd));
